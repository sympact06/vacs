...
local function add_ban(plr)
    ...
    local success, response = HttpService:PostAsync("https:/api.aero.nu/v1/roblox/vacs/lua/ban", HttpService:JSONEncode(banData))
    if not success then
        warn("Failed to ban player: Maybe vACS Servers are dead " .. response)
        return
    end
